...
const IconButton: FC<Props> = ({
  ...
  const classWrapper = classNames(
    className,
    {
      'bg-blue-500 text-white py-3 px-4 rounded-lg shadow hover:bg-blue-600 focus-visible:ring-offset-2':
        !onlyIcon,
      'text-current p-2 rounded-full border-gray-300 border hover:bg-gray-50 shadow-sm justify-center aspect-square':
        onlyIcon,
    ...
    <>
      {disabled || loading ? (
        ...
        >
          {Icon && (
            <Icon
              className={classNames(classIcon, { 'animate-pulse': loading })}
            />
          )}
