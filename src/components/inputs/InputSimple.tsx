  ...
  return (
    <InputWrapper
      ...
      render={({ classNameError, controllerProps }) => {
        const { onChange, value, ...controllerFieldProps } =
        ...
        return (
          <input
            className={classNames(
              classNameError,
              'mt-1 block w-full min-w-0 rounded-md bg-white text-gray-900 focus:border-blue-500 focus:ring-blue-500',
              {
                shadow: theme === 'filled',
            ...
            onChange={({ target: { value } }) =>
              onChange(
                parseOutput(
                  value,
                  inputProps?.type || inputPropsFromZod?.type || 'text'
                )
              )
            }
            value={parseInput(
...
export const makeCustomInputSimple = <
  TFieldValues extends FieldValues,
  TName extends FieldPath<TFieldValues>,
  TZodSchema extends ZodRawShape
>(rebundantProps: {
  ...
  switch (type) {
    case 'datetime-local': {
      if (!(value instanceof Date)) {
        console.warn(
          `Input with type datetime-local recieved a value that's not a date. Setting it to "".`,
          value
        )
        return ''
      }
