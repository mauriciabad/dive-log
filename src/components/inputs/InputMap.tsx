...
export interface Props<
  ...
  TNameLatitude extends FieldPath<TFieldValues>,
  TNameLongitude extends FieldPath<TFieldValues>,
...
>({
  ...
  const latitudeController = useController({
    control,
    name: internalLabelLatitude,
  })
  const longitudeController = useController({
    control,
    name: internalLabelLongitude,
  })

  const errorMessage = getErrorMessage(errorLatitude, errorLongitude)

  const { required: requiredLatitude } = getInputAttributesFromZod(
    schema,
    internalLabelLatitude
  )
  const { required: requiredLongitude } = getInputAttributesFromZod(
    schema,
    internalLabelLongitude
  )
  ...
  return (
    ...
    >
      <Map
        ...
        value={
          latitudeController.field.value && longitudeController.field.value
            ? {
...
export const makeCustomInputMap = <
  TFieldValues extends FieldValues,
  TNameLatitude extends FieldPath<TFieldValues>,
  TNameLongitude extends FieldPath<TFieldValues>,
  TZodSchema extends ZodRawShape
