...
const Profile: CustomNextPage = () => {
  ...
  return <>
    ...
    <h3 className="text-2xl font-bold mt-6">Dives list</h3>
    <IconButton href="/user/dives" text="Go to my dives" Icon={TbBook} />


    {!dives ? <p className="text-2xl">Loading dives...</p> : <ul>
      {dives.map((dive) => <li key={dive.id}><span>#{dive.diveNumber}</span> at<span>{dive.startDateTime.toLocaleString()}</span></li>)}
    </ul>
    }
  </>
}
